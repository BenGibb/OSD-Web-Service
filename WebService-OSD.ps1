...
Timeline:
    20/12/2018  -   v0.1.0  >   27/03/2019  -   v0.8.1  -   By Diagg/OSD-Couture.com Put together + all the rest by Diagg/OSD-Couture.com
    01/09/2019  -   v0.8.1  >   ??/??/????  -   v?.?.?  -   By Ben Gibb https://github.com/BenGibb/OSD-Web-Service

History:
    07/12/2018  -   v0.1.0  -   Initial Release
                -   V0.2.0  -   Added MDT output
                            -   Added Post support
                            -   Service State and request are logged in Event log
                -   v0.3.0  -   Added Ping
                            -   Added multi listener auto registering
    07/03/2019  -   v0.4.0  -   Added Basic Active Directory Support: GETPCINFO, ADDNEWPC, ADDGENERATEDPC, ADDPC2OU, ADDPC2GROUP
    15/03/2019  -   v0.5.0  -   Added Basic SCCM support: ADDPC2COLLECTION
    20/03/2019  -   v0.6.0  -   Refactored to support Asynchronious Web Requests (Boy that was hot!!!)
    22/03/2019  -   v0.7.0  -   TCP port to use is retived from firewall rule created from the installer
    26/03/2019  -   v0.8.0  -   Heavy code refactoring (4 lines changed) to avoid a monstruous memory leak !!!
...
$Script:LogFile = Initialize-Logging -oPath "C:\ProgramData\MDT-Manager\$CurrentUser\Logs\WebService-Status.log"
...
##== Check Port
If ([String]::IsNullOrWhiteSpace($Port) -or ($port -eq "Any")) {
    Write-ScriptEvent -value "[ERROR] No specified Port in firewall rules, pleas re-run installer!" -severity 3
    Exit
}
If (Get-NetTCPConnection -State Listen -LocalPort $Port -ErrorAction SilentlyContinue) {
    Write-ScriptEvent -value "[ERROR] Port $Port is already in use, please pick another one!" -severity 3
    Exit
}
...
Write-ScriptEvent -value "Registering listener: $prefix"
...
Catch {
    ...
    Write-ScriptEvent -value $Message -severity 3
...
Write-ScriptEvent -value $Message
Write-ScriptEvent -value "Listening on port $port..."
...
$requestListener = {
    ...
    If ($request.httpMethod -eq 'POST') {
        ...
        if ($null -eq $Data) {
            $commandOutput = "[ERROR] Bad request formating, use syntax: command=<string> format=[JSON|TEXT|XML|NONE|CLIXML]"
            $Format = "TEXT"
        }
    }
    ...
    { $Format = "MDTXML" }
    Write-ScriptEvent -value "Output Format = $Format"


    ##== Process requests
    If (-not [String]::IsNullOrWhiteSpace($Data)) {
        foreach ($kvp in $Data.GetEnumerator()) {
            Write-ScriptEvent -value $("Recieved " + $kvp.Key + " = " + $kvp.Value)
        }
    ...
    $commandOutput = switch ($Format.ToUpper()) {
        ...
        JSON {
            $commandOutput | ConvertTo-JSON
            break
        }
        CSV {
            $result = $null
            ($commandOutput | ConvertTo-csv -delimiter ";" -NoTypeInformation) | ForEach-Object { $result += $_ + "###" }
            [string]$result
            break
        }
        XML {
            $commandOutput | ConvertTo-XML -As String
            break
        }
        MDTXML {
            Convert-ObjectToXml -obj $commandOutput -tag 'MDT'
            break
        }
        CLIXML {
            [System.Management.Automation.PSSerializer]::Serialize($commandOutput)
            break
        }
        default {
            "Invalid output format selected, valid choices are TEXT, JSON, CSV, XML, MDTXML and CLIXML"
            $statusCode = 501
            break
        }
    }
    ...
    ##== Responding
    Write-ScriptEvent -value "Response:"
    If ([String]::IsNullOrWhiteSpace($commandOutput)) {
        Write-ScriptEvent -value "No data to send, back !!"
    }
    Else {
        Write-ScriptEvent -value $commandOutput
    }
...
while ($listener.IsListening) {
    ...
    If ($PreviousTime.Minutes -ne $oTimeLapse.Minutes) {
        ...
        If ($oTimeLapse.Minutes % 10 -eq 0) {
            Write-ScriptEvent -value "$((Get-Date).ToShortDateString()) - $((Get-Date).ToShortTimeString()) - The process is still running since $($oTimeLapse.days) Days - $($oTimeLapse.Hours) Hours - $($oTimeLapse.Minutes) Minutes - $($oTimeLapse.Seconds) Seconds"
            Write-ScriptEvent -value "Average Memory consumption is: $((Get-Process -Id $pid).WS/1MB) MB" 
        }
    }
}
...
$listener.Close()
Write-ScriptEvent -value 'Terminated...'