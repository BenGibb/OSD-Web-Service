...
History:
*   01/09/2019  -   v0.1.0  -   Initial Fork
                -   v0.1.1  -   Code cleanup and standardisation
#>
...
Function Read-GetRequest($request) {
    $data = @{ }
    $request.QueryString | ForEach-Object {
        $data.add($_ , $request.QueryString.get_Item($_))
    }
    return $data
}

Function Get-Time {
    $Result = Get-Date | Select-Object Date, Day, DayOfWeek, DayOfYear, Hour, Kind, Millisecond, Minute, Month, Second, Ticks, TimeOfDay, Year, DateTime
    Return $Result
}
...
function New-ScriptBlockCallback {
    ...
    if (!( 'CallbackEventBridge' -as [type])) {
        Add-Type @'
                using System;
...
'@
    ...
    Register-ObjectEvent -InputObject $bridge -EventName callbackcomplete -Action $Callback -MessageData $args | Out-Null
    $bridge.Callback
}
...
Function Add-NewGeneratedComputer {
    ...
    If (!([String]::IsNullOrWhiteSpace($Prefix))) {
        $AdFilter = "$Prefix*"
        [Int]$TmpLength = $Prefix.Length
    }
    If (!([String]::IsNullOrWhiteSpace($Suffix))) {
        $AdFilter = "*$Suffix"
        [Int]$TmpLength = $Suffix.Length
    }
    ...
    Do {
        ...
        If ([String]::IsNullOrWhiteSpace($NewComputerName)) {
            $NewComputerName = $($AdFilter.replace("*", $DigitsAsString))
        }

        ##== Cleanup name from strings to allow number incrementation
        If (!([String]::IsNullOrWhiteSpace($Prefix))) {
            $NewComputerName = $NewComputerName.replace($Prefix, "")
        }
        If (!([String]::IsNullOrWhiteSpace($Suffix))) {
            $NewComputerName = $NewComputerName.replace($Suffix, "")
        }

        ##== Increment number, and format with requested Digits
        [int]$NewComputerName = $NewComputerName
        $NewComputerName += $I
        [String]$NewComputerName = $Digits -f $NewComputerName

        ##== Rebuild new name
        If (!([String]::IsNullOrWhiteSpace($Prefix))) {
            $NewComputerName = $($Prefix + $NewComputerName)
        }
        If (!([String]::IsNullOrWhiteSpace($Suffix))) {
            $NewComputerName = $($NewComputerName + $Suffix)
        }
...
Function Add-ComputerToOU {
    ...
    If ($CompResult.Success -eq $False) {
        Return $CompResult
    }

    ##== Check OU
    $OuResult = Get-OuName -OuName $OU
    If ($OuResult.Success -eq $False) {
        Return $OuResult
    }
...
Function Get-CMComputerCollection {
    ...
    ##== INIT
    If ([String]::IsNullOrWhiteSpace($Script:CMDrive)) {
        $Script:CMDrive = Get-CMSite
    }
    If ([String]::IsNullOrWhiteSpace($Script:CurrentLocation)) {
...
Function Get-CMComputerName {
    ...
    If ([String]::IsNullOrWhiteSpace($Result)) {
        ...
        If (![String]::IsNullOrWhiteSpace($CollectionsMemberchip)) {
            Add-Member -InputObject $ReturnObj -MemberType NoteProperty -Name ComputerCMCollectionMemberchip -Value $($CollectionsMemberchip -join ",")
        }
        If (![String]::IsNullOrWhiteSpace($ClientActiveStatus)) {
            Add-Member -InputObject $ReturnObj -MemberType NoteProperty -Name ComputerCMClientIsActive -Value $([system.convert]::ToBoolean($ClientActiveStatus))
        }
        Set-Location $Script:CurrentLocation
        Return $ReturnObj
    }
